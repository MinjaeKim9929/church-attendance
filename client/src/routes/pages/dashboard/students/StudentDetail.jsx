...
export default function StudentDetail() {
	...
	const getFilteredRecords = () => {
		...
		if (dateFilter === 'last30') {
			...
			filtered = filtered.filter((record) => parseLocalDate(record.date) >= thirtyDaysAgo);
		} else if (dateFilter === 'last90') {
			const ninetyDaysAgo = new Date(now.getTime() - 90 * 24 * 60 * 60 * 1000);
			filtered = filtered.filter((record) => parseLocalDate(record.date) >= ninetyDaysAgo);
		} else if (dateFilter === 'currentYear') {
			const currentYear = now.getFullYear();
			filtered = filtered.filter((record) => parseLocalDate(record.date).getFullYear() === currentYear);
		}
														...
														<table className="w-full">
															<thead className="bg-gray-50 border-b border-gray-200">
																<tr>
																	...
																	<th className="px-4 py-3 text-left text-xs font-semibold text-gray-700">기록자</th>
																</tr>
															</thead>
																...
																{getPaginatedRecords().map((record, index) => (
																	<tr key={index} className="hover:bg-gray-50">
																		<td className="px-4 py-3 text-sm text-gray-900">
																			{parseLocalDate(record.date).toLocaleDateString('ko-KR', {
																				year: 'numeric',
																				month: 'long',
																				day: 'numeric',
																			})}
													...
													<div className="sm:hidden space-y-3">
														{getPaginatedRecords().map((record, index) => (
															<div key={index} className="bg-white border border-gray-200 rounded-lg p-4">
																...
																<p className="text-xs text-gray-500">기록자: {record.recordedBy?.fullName || '-'}</p>
															</div>
														))}
													</div>
